...
print("  - The server store carries NO IPv4 / MAC / hostname, BUT it DOES retain a")
print("    stable per-instance reporter_id linked to the indicators/hour it reported.")
print("  - The public feed exposes the indicator + an aggregate source COUNT, no ids.")
print("  So sections 1-3 answer 'what if the source hash leaked?' — it doesn't, and")
print("  even if it did, the salt makes it non-reversible.")
print("\n" + line)
print("CONCLUSION: The per-source HMAC is irreversible, AND source IP / MAC / hostname")
print("never leave the node. But this is NOT anonymity. The threat-network server")
print("INTENTIONALLY stores a stable reporter_id <-> indicator/hour linkage, so")
print("contributions are PSEUDONYMOUS — linkable to a per-instance pseudonym over")
print("time (the server can tell the same reporter reported X at hour H and Y at H+3),")
print("NOT anonymous. Retention/expiry of that linkage is incomplete today. See")
print("PRIVACY.md for the full residual model.")
print(line)
