...
func (db *DB) PublishCorpusProfile(ctx context.Context, profileID string, req corpus.PublishRequest, meta CorpusMutation) (*corpus.Profile, error) {
	...
	if err = requireExpectedCorpusRevision(ctx, tx, req.ExpectedCorpusRevision); err != nil {
		return nil, err
	}
	...
	if err = validateCorpusPublishReadiness(ctx, tx, profileID); err != nil {
		return nil, err
	}
	...
	if draftProfileID.Valid {
		if publishedProfileID.Valid {
			if _, err = tx.ExecContext(ctx, `UPDATE device_corpus_profile_revisions SET status = 'superseded'
                WHERE profile_revision_id = ?`, publishedProfileID.String); err != nil {
				return nil, err
			}
		}
		if _, err = tx.ExecContext(ctx, `UPDATE device_corpus_profile_revisions
            SET status = 'published', published_at = ? WHERE profile_revision_id = ?`, now, draftProfileID.String); err != nil {
			return nil, err
		}
	}

	rows, err := tx.QueryContext(ctx, `SELECT v.variant_id, draft.variant_revision_id,
	...
	for _, p := range promotions {
		if p.publishedID.Valid {
			if _, err = tx.ExecContext(ctx, `UPDATE device_corpus_variant_revisions SET status = 'superseded'
                WHERE variant_revision_id = ?`, p.publishedID.String); err != nil {
				return nil, err
			}
		}
		if _, err = tx.ExecContext(ctx, `UPDATE device_corpus_variant_revisions
            SET status = 'published', published_at = ? WHERE variant_revision_id = ?`, now, p.draftID); err != nil {
			return nil, err
		}
	}

	revision, snapshotHash, err := createCorpusReleaseTx(ctx, tx, now)
	if err != nil {
		return nil, err
	}
	...
	if err = insertCorpusAudit(ctx, tx, auditID, meta, "profile", profileID, "publish", reason,
...
func validateCorpusPublishReadiness(ctx context.Context, tx *sql.Tx, profileID string) error {
	...
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(DISTINCT v.variant_id)
		FROM device_corpus_variants v
		JOIN device_corpus_variant_revisions vr ON vr.variant_id = v.variant_id
	...
	return validateCorpusPublishQuality(ctx, tx, profileID)
}
...
func (db *DB) RetireCorpusProfile(ctx context.Context, profileID string, req corpus.LifecycleRequest, meta CorpusMutation) (*corpus.Profile, error) {
	reason, err := corpus.ValidateReasonCode(req.ReasonCode)
	if err != nil {
		return nil, corpusValidationError(err)
	}
	...
	after := before
	if published > 0 {
		revision, snapshotHash, releaseErr := createCorpusReleaseTx(ctx, tx, now)
		if releaseErr != nil {
	...
	if err = insertCorpusAudit(ctx, tx, auditID, meta, "profile", profileID, "retire", reason,
...
func (db *DB) WithdrawCorpusVariant(ctx context.Context, variantID string, req corpus.LifecycleRequest, meta CorpusMutation) (*corpus.Profile, error) {
	reason, err := corpus.ValidateReasonCode(req.ReasonCode)
	if err != nil {
		return nil, corpusValidationError(err)
	}
	...
	if err = ensureNoActiveCorpusVariantChildren(ctx, tx, variantID); err != nil {
		return nil, err
	}
	if err = requireExpectedCorpusRevision(ctx, tx, req.ExpectedCorpusRevision); err != nil {
		return nil, err
	...
	if err = insertCorpusAudit(ctx, tx, auditID, meta, "variant", variantID, "withdraw", reason,
		before, after, revisionPtr, now); err != nil {
		return nil, err
	}
	if err = tx.Commit(); err != nil {
...
func (db *DB) DiscardCorpusVariantDraft(ctx context.Context, variantID string, req corpus.LifecycleRequest, meta CorpusMutation) (*corpus.Profile, error) {
	reason, err := corpus.ValidateReasonCode(req.ReasonCode)
	if err != nil {
		return nil, corpusValidationError(err)
	}
	...
	var draftID string
	if err = tx.QueryRowContext(ctx, `SELECT variant_revision_id FROM device_corpus_variant_revisions
	...
	var published int
	if err = tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM device_corpus_variant_revisions
...
func ensureNoActiveCorpusVariantChildren(ctx context.Context, q corpusQuerier, variantID string) error {
	var descendants int
	err := q.QueryRowContext(ctx, `SELECT COUNT(DISTINCT child.variant_id)
