...
func TestNormalizeSourcesEnforcesCitationPrivacyContract(t *testing.T) {
	...
	got, err := NormalizeSources([]Source{{
		SourceID: "source_server_1", Kind: "vendor_doc", Title: "Example Camera Support",
	...
	if got[0].SourceID != "source_server_1" {
...
func TestNormalizeSourcesAndFactsRejectDuplicatesAndPreserveIDs(t *testing.T) {
	...
	fact := VersionFact{
		FactID: "fact_server_1", Attribute: "firmware_version", Relation: "exact",
		Value: "2.4.1", ConfidenceBP: 9000, SourceID: "source_server_1",
	}
	...
	duplicate := fact
	duplicate.FactID = "fact_server_2"
