...
('sim-fp-001', datetime('now', '-12 minutes'), 'dns_query', 'sim-macbook-pro', '192.0.2.42', 'fp-update.apple.com', 'A', '198.51.100.11', 0, 0.09, '["dns_query"]', 'Apple', 'default', 'local', 'DNS query to known Apple update/telemetry domain (excluded from DGA/Beaconing)', '{"device_context":{"boosts":[]}}', 0),
('sim-fp-002', datetime('now', '-11 minutes'), 'dns_query', 'sim-macbook-pro', '192.0.2.42', 'fp-telemetry.google.com', 'A', '198.51.100.12', 0, 0.07, '["dns_query"]', 'Apple', 'default', 'local', 'DNS query to known Google telemetry (conservative single-signal cap + known-good exclusion)', '{"device_context":{"boosts":[]}}', 0),
('sim-fp-003', datetime('now', '-10 minutes'), 'dns_query', 'sim-windows-laptop', '192.0.2.55', 'fp-ms-update.windows.com', 'A', '198.51.100.13', 0, 0.11, '["dns_query"]', 'Dell', 'default', 'local', 'Normal Microsoft/Windows update traffic from trusted device', '{"device_context":{"boosts":[]}}', 0);
...
-- ============================================================
INSERT INTO events (event_id, timestamp, event_type, source_hash, source_ip, domain, query_type, resolved_ip, blocked, anomaly_score, tags, device_vendor, network_segment, dns_source, threat_desc, metadata, acknowledged) VALUES
('sim-mid-001', datetime('now', '-9 minutes'), 'dns_query', 'sim-new-esp32', '192.0.2.201', 'mid-iot-checkin.example', 'A', '198.51.100.21', 0, 0.44, '["dns_query","new_device_context","iot_context","public_dns"]', 'Espressif', 'iot', 'public', 'New IoT device (first seen <48h) on iot segment using public DNS resolver (device context boosts applied)', '{"device_context":{"boosts":[{"reason":"new_device (<48h) +0.15","delta":0.15},{"reason":"iot_segment +0.12","delta":0.12},{"reason":"public_dns_from_high_risk +0.12","delta":0.12}]}}', 0),
('sim-mid-002', datetime('now', '-8 minutes'), 'dns_query', 'sim-new-esp32', '192.0.2.201', 'mid-iot-telemetry.example', 'A', '198.51.100.22', 0, 0.39, '["dns_query","new_device_context","iot_context"]', 'Philips Lighting BV', 'iot', 'public', 'Synthetic new Philips Hue-like device on an IoT segment performing repeated lookups (moderate risk + context)', '{"device_context":{"boosts":[{"reason":"new_device (<48h) +0.15","delta":0.15},{"reason":"iot_segment +0.12","delta":0.12}]}}', 0);
