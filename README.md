...
- **Advanced DNS Hunting** is an off-by-default, per-Core profile for behavioural DGA/NXDOMAIN bursts, beaconing, tunneling, rebinding, and public-resolver-bypass detections. Exact threat-intelligence matches remain active independently.
...
(
  ...
  cleanup_vedetta_install() { /bin/rm -f -- "$INSTALLER_FILE" "$CHECKSUM_FILE" "$CODE_FILE"; }
  trap cleanup_vedetta_install EXIT
  /usr/bin/curl -fsSL -o "$INSTALLER_FILE" "${RELEASE_BASE}/install.sh"
  /usr/bin/curl -fsSL -o "$CHECKSUM_FILE" "${RELEASE_BASE}/checksums.txt"
  EXPECTED="$(/usr/bin/awk 'NF == 2 && $2 == "install.sh" { print $1; found++ } END { if (found != 1) exit 1 }' "$CHECKSUM_FILE")"
  case "$EXPECTED" in ''|*[!0-9a-fA-F]*) printf 'Invalid install.sh checksum manifest\n'; exit 1 ;; esac
  [ "${#EXPECTED}" -eq 64 ] || { printf 'Invalid install.sh checksum length\n'; exit 1; }
  if [ -x /usr/bin/sha256sum ]; then
    ACTUAL="$(/usr/bin/sha256sum "$INSTALLER_FILE" | /usr/bin/awk '{print $1}')"
  elif [ -x /usr/bin/shasum ]; then
    ACTUAL="$(/usr/bin/shasum -a 256 "$INSTALLER_FILE" | /usr/bin/awk '{print $1}')"
  else
    printf 'A SHA-256 utility is required\n'; exit 1
  fi
...
$Entries = @(Get-Content -LiteralPath checksums.txt | Where-Object { $_ -match '^[0-9a-fA-F]{64}\s+\*?install\.ps1$' })
if ($Entries.Count -ne 1) { throw "checksums.txt must contain exactly one install.ps1 entry" }
...
if ($Actual -ne $Expected) { throw "install.ps1 checksum verification failed" }
...
- [Local Device-Corpus Matching](docs/device-corpus-matching.md)
- [Device-Fingerprint Contribution (research gate; no transport)](docs/device-fingerprint-contribution.md)
