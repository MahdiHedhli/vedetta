...
func (s *Scanner) ScanContext(ctx context.Context, cidr string, withPorts bool) (*ScanResult, error) {
	...
	// Treat child XML as untrusted. Nmap was invoked with one validated IPv4
	// target, so discard every host it reports outside that exact target before
	// neighbor enrichment, follow-up probing, or returning anything to Core.
	filterScanResultToIPv4Target(discovery, cidr)
	...
	if withPorts {
		...
		for offset := 0; offset < len(hosts); offset += portScanBatchSize {
			...
			filterScanResultToIPv4Allowlist(ports, hosts[offset:end])
...
func discoveredIPs(hosts []DiscoveredHost) []string {
	...
	for _, host := range hosts {
		if ip := net.ParseIP(host.IPAddress); ip != nil && ip.To4() != nil && !strings.Contains(host.IPAddress, ":") {
			seen[ip.To4().String()] = struct{}{}
		}
	}
...
func filterScanResultToIPv4Target(result *ScanResult, target string) {
	if result == nil {
		return
	}
	...
	for _, host := range result.Hosts {
		...
		if ip == nil || ip.To4() == nil || strings.Contains(host.IPAddress, ":") || !targetContainsIPv4(target, host.IPAddress) {
			continue
		}
...
func filterScanResultToIPv4Allowlist(result *ScanResult, allowed []string) {
	if result == nil {
		return
	}
	...
	for _, raw := range allowed {
		if ip := net.ParseIP(raw); ip != nil && ip.To4() != nil && !strings.Contains(raw, ":") {
			allow[ip.To4().String()] = struct{}{}
		}
	}
	filtered := result.Hosts[:0]
	for _, host := range result.Hosts {
		ip := net.ParseIP(host.IPAddress)
		if ip == nil || ip.To4() == nil || strings.Contains(host.IPAddress, ":") {
			continue
		}
...
func normalizeNeighbor(ip, rawMAC string) (string, bool) {
	...
	// macOS arp(8) omits leading zeroes from individual octets. ParseMAC quite
	// correctly rejects that non-canonical spelling, so normalize exactly six
	// one-or-two-digit colon-separated octets before validating the address.
	if parts := strings.Split(rawMAC, ":"); len(parts) == 6 {
		for i, part := range parts {
			if len(part) == 1 {
				parts[i] = "0" + part
			}
		}
		rawMAC = strings.Join(parts, ":")
	}
...
var darwinARPLine = regexp.MustCompile(`\(([0-9]+(?:\.[0-9]+){3})\)\s+at\s+([0-9A-Fa-f]{1,2}(?::[0-9A-Fa-f]{1,2}){5})\s+on\s+\S+`)
...
func parseNmapXMLDetailed(data []byte, scanTime time.Time, duration time.Duration) (*ScanResult, bool, error) {
	...
	for _, h := range run.Hosts {
		...
		for _, addr := range h.Addresses {
			...
			case "ipv4":
				if ip := net.ParseIP(addr.Addr); ip != nil && ip.To4() != nil && !strings.Contains(addr.Addr, ":") {
					host.IPAddress = ip.To4().String()
				}
			case "mac":
