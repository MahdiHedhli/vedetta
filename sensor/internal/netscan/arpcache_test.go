...
func TestParseProcNetARP(t *testing.T) {
	...
	// incomplete (0x0) row, an IPv4-multicast row, a directed-broadcast row, an
	// all-ones-MAC row, and a short/garbage line.
...
`
	...
	want := map[string]neighbor{
		"192.0.2.10": {ip: "192.0.2.10", mac: "aa:bb:cc:dd:ee:01", iface: "eth0"},
		"192.0.2.11": {ip: "192.0.2.11", mac: "aa:bb:cc:dd:ee:02", iface: "eth0"},  // uppercase normalized
		"192.0.2.14": {ip: "192.0.2.14", mac: "aa:bb:cc:dd:ee:04", iface: "wlan0"}, // permanent (0x6) kept
	}
	if len(got) != len(want) {
