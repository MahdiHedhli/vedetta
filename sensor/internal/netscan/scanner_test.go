...
func TestScanResultFilteringRejectsIPv6AndOutOfTargetHosts(t *testing.T) {
	result := &ScanResult{Hosts: []DiscoveredHost{
		{IPAddress: "192.0.2.9", Status: "up"},
		{IPAddress: "198.51.100.9", Status: "up"},
		{IPAddress: "2001:db8::9", Status: "up"},
		{IPAddress: "::ffff:192.0.2.10", Status: "up"},
		{IPAddress: "not-an-ip", Status: "up"},
	}}
	filterScanResultToIPv4Target(result, "192.0.2.0/24")
	if got := discoveredIPs(result.Hosts); !reflect.DeepEqual(got, []string{"192.0.2.9"}) {
		t.Fatalf("target-filtered hosts = %#v, want only requested IPv4", result.Hosts)
	}

	ports := &ScanResult{Hosts: []DiscoveredHost{
		{IPAddress: "192.0.2.9", OpenPorts: []int{443}},
		{IPAddress: "192.0.2.10", OpenPorts: []int{22}},
		{IPAddress: "2001:db8::9", OpenPorts: []int{80}},
	}}
	filterScanResultToIPv4Allowlist(ports, []string{"192.0.2.9"})
	if len(ports.Hosts) != 1 || ports.Hosts[0].IPAddress != "192.0.2.9" {
		t.Fatalf("allowlisted port results = %#v", ports.Hosts)
	}
}

func TestParseNmapXMLIgnoresIPv6Addresses(t *testing.T) {
	xml := []byte(`<nmaprun>
  <host><status state="up"/><address addr="2001:db8::9" addrtype="ipv6"/></host>
  <runstats><finished elapsed="0" exit="success"/></runstats>
</nmaprun>`)
	result, _, err := parseNmapXMLDetailed(xml, time.Now(), 0)
	if err != nil {
		t.Fatal(err)
	}
	if len(result.Hosts) != 0 {
		t.Fatalf("IPv6 child output was accepted: %#v", result.Hosts)
	}
}
...
<nmaprun>
  ...
  <host><status state="up"/><address addr="198.51.100.9" addrtype="ipv4"/></host>
  <host><status state="up"/><address addr="2001:db8::9" addrtype="ipv6"/></host>
...
func TestDarwinNeighborParserNormalizesSingleDigitMACOctets(t *testing.T) {
	line := []byte("? (192.0.2.8) at 0:0:5e:0:53:8 on en0 ifscope [ethernet]")
	match := darwinARPLine.FindSubmatch(line)
	if len(match) != 3 {
		t.Fatalf("Darwin arp line did not match: %q", line)
	}
	mac, ok := normalizeNeighbor(string(match[1]), string(match[2]))
	if !ok || mac != "00:00:5e:00:53:08" {
		t.Fatalf("normalized Darwin neighbor = %q, %v", mac, ok)
	}
}
