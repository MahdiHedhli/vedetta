...
func rtMsg(flags int, ip [4]byte, mac []byte, iface string) *route.RouteMessage {
	...
	addrs[unix.RTAX_GATEWAY] = &route.LinkAddr{Name: iface, Addr: mac}
...
func TestNeighborsFromRIB(t *testing.T) {
	...
	want := map[string]neighbor{
		"192.0.2.10": {ip: "192.0.2.10", mac: "aa:bb:cc:dd:ee:01", iface: "en0"},
		"192.0.2.20": {ip: "192.0.2.20", mac: "00:00:5e:00:53:20", iface: "en1"},
	}
	if len(got) != len(want) {
