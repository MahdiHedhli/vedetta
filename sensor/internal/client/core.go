...
func (c *CoreClient) persistToken(rawToken string) error {
	...
	committed = true
	// The atomic rename has committed the new credential. Update memory before
	// the durability sync so a sync error cannot leave this process sending the
	// now-revoked prior token while disk already contains the replacement.
	c.tokenMu.Lock()
	c.authToken = token
	c.tokenMu.Unlock()
	if err := c.syncPersistedTokenDirectory(dir); err != nil {
		return fmt.Errorf("sync sensor token directory: %w", err)
	}
	return nil
}

func (c *CoreClient) syncPersistedTokenDirectory(path string) error {
	if c.directorySync != nil {
		return c.directorySync(path)
	}
	return syncTokenDirectory(path)
}
