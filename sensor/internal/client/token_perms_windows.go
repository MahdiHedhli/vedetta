...
// SeTakeOwnershipPrivilege is process-token state, not goroutine-local state.
// Serialize every enable/use/restore sequence so concurrent CoreClient instances
// cannot save and restore overlapping snapshots and leave the process token in
// the wrong state.
var takeOwnershipPrivilegeMu sync.Mutex
...
func securePath(path string, isDir bool) error {
	...
	if err := withTakeOwnershipPrivilege(path, func() error {
		// Excluding FILE_SHARE_DELETE keeps the name bound to this object until both
		// ownership and DACL changes complete. OPEN_REPARSE_POINT makes the final
		// path component inspectable rather than following a symlink/junction.
...
// withTakeOwnershipPrivilege runs fn while SeTakeOwnershipPrivilege is enabled
// and restores the exact prior process-token state before another caller may
// enter. A restoration failure is never hidden by an earlier operation error.
func withTakeOwnershipPrivilege(path string, fn func() error) (err error) {
	takeOwnershipPrivilegeMu.Lock()
	defer takeOwnershipPrivilegeMu.Unlock()
	...
	defer func() {
		if restoreErr := restorePrivilege(); restoreErr != nil {
			err = errors.Join(err,
				fmt.Errorf("restore token privileges after securing %s: %w", path, restoreErr))
		}
	}()
...
func openNonReparsePath(path string, access uint32) (windows.Handle, error) {
	return openNonReparsePathWithShare(path, access, windows.FILE_SHARE_READ|windows.FILE_SHARE_WRITE)
}

func openNonReparsePathWithShare(path string, access, share uint32) (windows.Handle, error) {
	if err := validateNoReparseAncestry(path); err != nil {
		return windows.InvalidHandle, err
	}
	return openSingleNonReparsePathWithShare(path, access, share)
}

// validateNoReparseAncestry rejects junctions, mount-point reparse objects, and
// symlinks in every existing component, not merely the final path. This is a
// validation-at-each-operation defense, not a claim of handle-relative creation:
// the exact protected Vedetta leaf and trusted deployment ancestry remain the
// mutation boundary between validation and the subsequent operation.
func validateNoReparseAncestry(path string) error {
	absolute, err := filepath.Abs(path)
	if err != nil {
		return fmt.Errorf("resolve token path ancestry %s: %w", path, err)
	}
	absolute = filepath.Clean(absolute)
	volume := filepath.VolumeName(absolute)
	if volume == "" || !filepath.IsAbs(absolute) {
		return fmt.Errorf("token path must resolve to an absolute Windows path: %s", path)
	}
	...
	if remainder != "" {
		for _, component := range strings.Split(remainder, string(os.PathSeparator)) {
			if component == "" || component == "." || component == ".." {
				return fmt.Errorf("token path has an unsafe component: %s", path)
			}
	...
	for _, component := range components {
		handle, err := openSingleNonReparsePathWithShare(component, windows.FILE_READ_ATTRIBUTES,
			windows.FILE_SHARE_READ|windows.FILE_SHARE_WRITE|windows.FILE_SHARE_DELETE)
		if err != nil {
			return fmt.Errorf("token path component %s is unavailable or a reparse point: %w", component, err)
		}
		if err := windows.CloseHandle(handle); err != nil {
			return fmt.Errorf("close token path component %s: %w", component, err)
		}
	}
	return nil
}

func openSingleNonReparsePathWithShare(path string, access, share uint32) (windows.Handle, error) {
	pathPtr, err := windows.UTF16PtrFromString(path)
	if err != nil {
		return windows.InvalidHandle, err
	}
	handle, err := windows.CreateFile(pathPtr, access,
		share, nil, windows.OPEN_EXISTING,
...
func enableTakeOwnershipPrivilege() (func() error, error) {
	...
	// GetLastError is thread-local, while Go's Windows syscall bridge may resume
	// a goroutine on a different M after a call. Keep both calls on one OS thread
	// so ERROR_NOT_ALL_ASSIGNED is read from this AdjustTokenPrivileges call.
	runtime.LockOSThread()
	adjustErr := windows.AdjustTokenPrivileges(token, false, &desired,
	...
	// AdjustTokenPrivileges can report API success while assigning none of the
	// requested privileges. Its documented contract requires checking last-error
	// immediately for ERROR_NOT_ALL_ASSIGNED before executing privileged work.
	if err := adjustedPrivilegeAssignmentError(lastErr); err != nil {
		_ = token.Close()
		return nil, err
	}
	restored := false
...
func adjustedPrivilegeAssignmentError(lastErr error) error {
	if lastErr == nil {
		return nil
	}
	if errors.Is(lastErr, windows.ERROR_NOT_ALL_ASSIGNED) {
		return fmt.Errorf("SeTakeOwnershipPrivilege was not assigned: %w", lastErr)
	}
	return fmt.Errorf("enable SeTakeOwnershipPrivilege reported a last-error status: %w", lastErr)
}
