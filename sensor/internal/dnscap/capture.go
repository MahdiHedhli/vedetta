...
func (c *Capturer) parsePacket(packet gopacket.Packet) *Query {
	...
	if dns.QR {
		...
		// Note: ans.Name is the record's OWNER name (the queried domain), not the
		// answer data. For CNAME records the target lives in ans.CNAME; appending
		// ans.Name would pollute answers with the query domain itself.
		for _, ans := range dns.Answers {
			...
			case layers.DNSTypeCNAME:
				target := strings.TrimSuffix(string(ans.CNAME), ".")
				if target != "" {
					query.Answers = append(query.Answers, target)
				}
			}
			// Other record types (TXT, MX, SOA, ...) are intentionally skipped:
			// they carry no destination info useful for actionability.
		}
