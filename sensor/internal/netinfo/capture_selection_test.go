...
func TestRankCaptureCandidatesPrefersScanCIDRMatchOverTunnelRoute(t *testing.T) {
	scanNet := parseOptionalCIDR("192.0.2.0/24")
	...
	candidates := rankCaptureCandidates([]captureCandidate{
		...
		{
			Name:          "en0",
			IPs:           []net.IP{net.ParseIP("192.0.2.182")},
			IsPhysicalish: true,
		},
	}, scanNet, routeIP)
...
func TestRankCaptureCandidatesPenalizesVirtualMacInterfaces(t *testing.T) {
	candidates := rankCaptureCandidates([]captureCandidate{
		{
			Name:          "ap1",
			IPs:           []net.IP{net.ParseIP("192.0.2.50")},
			IsVirtual:     true,
			IsPhysicalish: false,
		},
		{
	...
	}, parseOptionalCIDR("192.0.2.0/24"), nil)
...
func TestFormatCaptureSelectionIncludesOverrideHint(t *testing.T) {
	sel := &CaptureSelection{
		Name:   "en0",
		Reason: "scan CIDR matches host IP 192.0.2.182, physical LAN/Wi-Fi interface",
