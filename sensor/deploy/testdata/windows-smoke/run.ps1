...
try {
    ...
    Write-Host "== reverse-proxy path trailing slash changes require credential migration =="
    $TrailingSlashPathName = "`"$ExePath`" --core `"$CoreURL/tenant/`" --cidr `"$CIDR`" --dns --passive-discovery=false"
    Set-SmokeServiceCommandLine -CommandLine $TrailingSlashPathName
    $BeforeTrailingSlash = Get-VedettaServiceState
    $BeforeTrailingSlashBinary = Get-FileSha256 $ExePath
    $BeforeTrailingSlashToken = Get-FileSha256 $TokenPath
    $TrailingSlashResult = Invoke-InstallerChild -Arguments @(
        "-Core", "$CoreURL/tenant",
        "-CIDR", $CIDR,
        "-Binary", $UpdatedSensor,
        "-BinarySha256", (Get-FileSha256 $UpdatedSensor)
    )
    Assert-True ($TrailingSlashResult.ExitCode -ne 0) "path-distinct Core URLs were treated as equivalent"
    Assert-True ($TrailingSlashResult.Output -match "credential migration") "path trailing-slash change did not fail at the Core migration guard"
    $AfterTrailingSlash = Get-VedettaServiceState
    Assert-Equal $AfterTrailingSlash.Status "Running" "path trailing-slash guard stopped the prior service"
    Assert-Equal $AfterTrailingSlash.ProcessId $BeforeTrailingSlash.ProcessId "path trailing-slash guard restarted the prior service"
    Assert-Equal $AfterTrailingSlash.PathName $BeforeTrailingSlash.PathName "path trailing-slash guard changed ImagePath"
    Assert-Equal (Get-FileSha256 $ExePath) $BeforeTrailingSlashBinary "path trailing-slash guard changed the binary"
    Assert-Equal (Get-FileSha256 $TokenPath) $BeforeTrailingSlashToken "path trailing-slash guard changed the token"
