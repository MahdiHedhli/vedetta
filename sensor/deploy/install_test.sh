...
echo "case: installer rejects IPv6 scan CIDR before mutation"
IPV6_CIDR_LOG="${WORK}/ipv6-cidr.log"
if VEDETTA_ALLOW_UNSAFE_TEST_PATHS=1 bash "$INSTALL_SH" \
     --core http://198.51.100.10:8080 --cidr 2001:db8::/64 --no-service >"$IPV6_CIDR_LOG" 2>&1; then
  echo "  FAIL: IPv6 scan CIDR was accepted"
  FAIL=$((FAIL + 1))
else
  echo "  ok: IPv6 scan CIDR rejected"
  PASS=$((PASS + 1))
fi
check "IPv6 rejection explains IPv4-only contract" "IPv4 CIDRs only" "$IPV6_CIDR_LOG"
...
run_installer() { # run_installer <plist-out> [extra install.sh args...]
  ...
  VEDETTA_BIN_DIR="${RUN_INSTALLER_BIN_DIR:-${WORK}/prefix}" \
...
echo "case: non-regular install destinations are rejected before promotion"
NONREG_BINARY_ROOT="${WORK}/non-regular-binary"
NONREG_BINARY_DEST="${NONREG_BINARY_ROOT}/vedetta-sensor"
mkdir -p "$NONREG_BINARY_DEST"
printf 'binary-destination-sentinel\n' >"${NONREG_BINARY_DEST}/sentinel"
if RUN_INSTALLER_BIN_DIR="$NONREG_BINARY_ROOT" \
   run_installer "${WORK}/non-regular-binary.plist" \
     --core http://198.51.100.10:8080 --no-service; then
  echo "  FAIL: directory binary destination was accepted"
  FAIL=$((FAIL + 1))
else
  echo "  ok: directory binary destination rejected"
  PASS=$((PASS + 1))
fi
check "binary destination rejection is explicit" \
  "sensor binary destination exists but is not a regular, non-symlink file" \
  "${WORK}/non-regular-binary.plist.log"
check "binary destination contents remain unchanged" \
  "binary-destination-sentinel" "${NONREG_BINARY_DEST}/sentinel"
if [ ! -e "${NONREG_BINARY_DEST}/candidate" ]; then
  echo "  ok: candidate was not moved inside the binary destination directory"
  PASS=$((PASS + 1))
else
  echo "  FAIL: candidate was moved inside the binary destination directory"
  FAIL=$((FAIL + 1))
fi

NONREG_SERVICE="${WORK}/non-regular-service.plist"
mkdir -p "$NONREG_SERVICE"
printf 'service-destination-sentinel\n' >"${NONREG_SERVICE}/sentinel"
printf 'already-enrolled-token\n' >"${NONREG_SERVICE}.token"
if run_installer "$NONREG_SERVICE" --core http://198.51.100.10:8080; then
  echo "  FAIL: directory service destination was accepted"
  FAIL=$((FAIL + 1))
else
  echo "  ok: directory service destination rejected"
  PASS=$((PASS + 1))
fi
check "service destination rejection is explicit" \
  "service definition destination exists but is not a regular, non-symlink file" \
  "${NONREG_SERVICE}.log"
check "service destination contents remain unchanged" \
  "service-destination-sentinel" "${NONREG_SERVICE}/sentinel"
if [ ! -e "${NONREG_SERVICE}/candidate" ]; then
  echo "  ok: candidate was not moved inside the service destination directory"
  PASS=$((PASS + 1))
else
  echo "  FAIL: candidate was moved inside the service destination directory"
  FAIL=$((FAIL + 1))
fi
...
if VEDETTA_TEST_FORCE_PLIST_FALLBACK=1 run_installer "$PLIST_CORE_EQ" --core "$CORE_EQ_REQUEST"; then
  echo "  ok: LaunchDaemon scheme and authority case differences accepted"
  PASS=$((PASS + 1))
else
...
echo "case: macOS non-root Core trailing slash change is rejected before mutation"
PLIST_CORE_SLASH="${WORK}/core-trailing-slash.plist"
write_launchd_service "$PLIST_CORE_SLASH" 'https://core.example.test/tenant/' core-slash-blocked
printf 'unchanged-launchd-slash-token' >"${PLIST_CORE_SLASH}.token"
if RUN_INSTALLER_LAUNCHD_STATE=loaded run_installer "$PLIST_CORE_SLASH" --core 'https://core.example.test/tenant'; then
  echo "  FAIL: path-distinct LaunchDaemon Core URL was accepted"
  FAIL=$((FAIL + 1))
else
  echo "  ok: path-distinct LaunchDaemon Core URL rejected"
  PASS=$((PASS + 1))
fi
check "LaunchDaemon trailing-slash error explains credential migration" "credential migration" "${PLIST_CORE_SLASH}.log"
check "rejected trailing-slash LaunchDaemon config stayed intact" "core-slash-blocked" "$PLIST_CORE_SLASH"
check "rejected trailing-slash LaunchDaemon token stayed intact" "unchanged-launchd-slash-token" "${PLIST_CORE_SLASH}.token"
check "rejected trailing-slash LaunchDaemon stayed loaded" "loaded" "$LAUNCHD_STATE"
refute "rejected trailing-slash LaunchDaemon was not stopped" "bootout" "$LAUNCHD_COMMANDS"
refute "rejected trailing-slash LaunchDaemon never enrolled" "argv=[--enroll-only" "${PLIST_CORE_SLASH}.senslog"
...
VEDETTA_SKIP_HEALTHCHECK=1 \
  bash "$INSTALL_SH" --core 'https://example.test/a%2Fb/$literal' --cidr 192.0.2.0/24 --enroll-code-file "$CODE4" >"${UNIT4}.log" 2>&1
...
check  "unit persists requested CIDR"      "192.0.2.0/24"              "$UNIT4"
...
printf 'active enabled\n' >"$SYSTEMD_STATE"
: >"${UNIT4}.senslog"
if run_linux_installer "$UNIT4" "${WORK}/prefix4" --core 'https://example.test/a%2Fb/$literal' --cidr 192.0.2.0/24; then
...
write_systemd_service "$UNIT_CORE_EQ" 'HTTP://CORE.Example.Test/' core-equivalent-old
printf 'equivalent-systemd-token' >"${UNIT_CORE_EQ}.token"
printf 'inactive disabled\n' >"$SYSTEMD_STATE"
: >"$SYSTEMD_COMMANDS"
if run_linux_installer "$UNIT_CORE_EQ" "${WORK}/prefix-core-equivalent" --core 'http://core.example.test'; then
  echo "  ok: systemd scheme/authority case and root-slash differences accepted"
  PASS=$((PASS + 1))
else
  echo "  FAIL: normalized-equivalent systemd Core URL rejected"
  FAIL=$((FAIL + 1))
fi
check "equivalent systemd update wrote requested Core" 'http://core.example.test' "$UNIT_CORE_EQ"

echo "case: systemd non-root Core trailing slash change is rejected before mutation"
UNIT_CORE_SLASH="${WORK}/core-trailing-slash.service"
write_systemd_service "$UNIT_CORE_SLASH" 'https://core.example.test/tenant/' core-slash-blocked
printf 'unchanged-systemd-slash-token' >"${UNIT_CORE_SLASH}.token"
printf 'active enabled\n' >"$SYSTEMD_STATE"
: >"$SYSTEMD_COMMANDS"
if run_linux_installer "$UNIT_CORE_SLASH" "${WORK}/prefix-core-trailing-slash" --core 'https://core.example.test/tenant'; then
  echo "  FAIL: path-distinct systemd Core URL was accepted"
  FAIL=$((FAIL + 1))
else
  echo "  ok: path-distinct systemd Core URL rejected"
  PASS=$((PASS + 1))
fi
check "systemd trailing-slash error explains credential migration" "credential migration" "${UNIT_CORE_SLASH}.log"
check "rejected trailing-slash systemd config stayed intact" "core-slash-blocked" "$UNIT_CORE_SLASH"
check "rejected trailing-slash systemd token stayed intact" "unchanged-systemd-slash-token" "${UNIT_CORE_SLASH}.token"
check "rejected trailing-slash systemd service stayed running and enabled" "active enabled" "$SYSTEMD_STATE"
refute "rejected trailing-slash systemd update was not stopped" "stop vedetta-sensor" "$SYSTEMD_COMMANDS"
refute "rejected trailing-slash systemd unit never enrolled" "argv=[--enroll-only" "${UNIT_CORE_SLASH}.senslog"
...
# Keep malformed fixtures non-IP-shaped so executable test data also satisfies
# the repository's strict no-network-identifier sanitation policy.
...
echo "case: fresh rollback removes a binary promoted before bookkeeping"
FRESHBINROOT="${WORK}/fresh-binary-interrupt"
FRESHBINPLIST="${FRESHBINROOT}/unused.plist"
mkdir -p "$FRESHBINROOT"
if VEDETTA_TEST_FAIL_AFTER_BINARY_MOVE=1 \
   RUN_INSTALLER_BIN_DIR="${FRESHBINROOT}/bin" \
   run_installer "$FRESHBINPLIST" \
     --core http://198.51.100.10:8080 --no-service; then
  echo "  FAIL: synthetic post-binary-move failure unexpectedly succeeded"
  FAIL=$((FAIL + 1))
else
  echo "  ok: synthetic post-binary-move failure triggered rollback"
  PASS=$((PASS + 1))
fi
if [ ! -e "${FRESHBINROOT}/bin/vedetta-sensor" ]; then
  echo "  ok: fresh promoted binary removed without relying on a replacement flag"
  PASS=$((PASS + 1))
else
  echo "  FAIL: fresh promoted binary survived rollback"
  FAIL=$((FAIL + 1))
fi

echo "case: fresh rollback removes config promoted before bookkeeping"
FRESHCONFIGROOT="${WORK}/fresh-config-interrupt"
FRESHCONFIGPLIST="${FRESHCONFIGROOT}/com.vedetta.sensor.plist"
mkdir -p "$FRESHCONFIGROOT"
printf 'preexisting-test-token' >"${FRESHCONFIGPLIST}.token"
if VEDETTA_TEST_FAIL_AFTER_CONFIG_MOVE=1 \
   RUN_INSTALLER_BIN_DIR="${FRESHCONFIGROOT}/bin" \
   run_installer "$FRESHCONFIGPLIST" --core http://198.51.100.10:8080; then
  echo "  FAIL: synthetic post-config-move failure unexpectedly succeeded"
  FAIL=$((FAIL + 1))
else
  echo "  ok: synthetic post-config-move failure triggered rollback"
  PASS=$((PASS + 1))
fi
if [ ! -e "$FRESHCONFIGPLIST" ] && [ ! -e "${FRESHCONFIGROOT}/bin/vedetta-sensor" ]; then
  echo "  ok: fresh promoted config and binary removed without replacement flags"
  PASS=$((PASS + 1))
else
  echo "  FAIL: fresh promoted config or binary survived rollback"
  FAIL=$((FAIL + 1))
fi
