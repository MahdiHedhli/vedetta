...
case "$SENSOR_CIDR" in
  auto) ;;
  *[!A-Fa-f0-9:./]*|'') die "--cidr must be 'auto' or a numeric IPv4/IPv6 CIDR" ;;
  */*) ;;
  *) die "--cidr must include a prefix length (for example 192.168.1.0/24)" ;;
esac
...
# Match the Core client's identity rules without decoding the path: scheme and
# authority are case-insensitive, reverse-proxy paths remain case-sensitive, and
# one or more trailing slashes do not change the base URL. Query strings,
# fragments, credentials, and hostless URLs are rejected above for the requested
# URL and again here for the value recovered from an existing service definition.
normalize_core_url() {
  local raw="$1" scheme rest authority path normalized
  case "$raw" in
...
rollback_install() {
  ...
  stop_service || { rollback_failed=1; restore_complete=0; }
	...
	if [ "$INSTALL_SERVICE" = true ]; then
		...
		elif [ "$CONFIG_REPLACED" = "1" ]; then
			$SUDO rm -f "$SERVICE_FILE" || { rollback_failed=1; restore_complete=0; }
		else
			$SUDO rm -f "$CONFIG_STAGE" || { rollback_failed=1; restore_complete=0; }
		fi
	fi
	...
	elif [ "$BIN_REPLACED" = "1" ]; then
		$SUDO rm -f "$BIN_DEST" || { rollback_failed=1; restore_complete=0; }
	else
		$SUDO rm -f "$BIN_STAGE" || { rollback_failed=1; restore_complete=0; }
  fi
...
$SUDO test -f "$BIN_DEST" && HAD_BINARY=1
if [ "$INSTALL_SERVICE" = true ] && $SUDO test -f "$SERVICE_FILE"; then
  HAD_CONFIG=1
fi
if [ "$INSTALL_SERVICE" = true ]; then
  if [ "$OS" = "Darwin" ]; then
