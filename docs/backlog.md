...
| VED-010 | Implement the opt-in telemetry service (spec 002) | 2026-07-03 | Off unless `VEDETTA_TELEMETRY_OPTIN=true` at delivery (default later flipped to on/opt-out via issue #37), structural identifier stripping, allowlisted aggregated-signal export (full wire/storage boundary in `PRIVACY.md`), bounded spool + backoff, cross-service contract tests. Operational dry-run → VED-014 |
