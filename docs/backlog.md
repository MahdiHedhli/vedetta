...
| VED-015 | Fix broken fresh-install migration runner (fails at 009) | Schema / Bug | P0 | Ready | Unassigned | The filesystem migration runner fails at `009_event_type_encrypted_dns.sql` ("14 columns but 19 values") on a fresh DB because `001_init.sql` was baked to the final schema, making 008/009 non-replayable. Go tests miss it (inline fallback). Fresh production Docker Core installs are affected. Harden 008/009 + add a real-runner regression test |
...
| VED-008 | Opt-in threat-intel MVP contract | 2026-07-03 | Superseded and delivered via [specs/002](../specs/002-telemetry-service/) + [specs/003](../specs/003-threat-network/); privacy-reduced, advisory-only, off by default |
| VED-010 | Implement the opt-in telemetry service (spec 002) | 2026-07-03 | Off unless `VEDETTA_TELEMETRY_OPTIN=true`, structural PII stripping, aggregate-only signed export, bounded spool + backoff, cross-service contract tests. Operational dry-run → VED-014 |
| VED-011 | Implement the community threat network (spec 003) | 2026-07-03 | Signed reporter ingest, idempotent per-reporter batches, whole-batch privacy/schema re-gate, Sybil-resistant consensus, advisory-only feed, swept rate limiter. Operational check → VED-014 |
