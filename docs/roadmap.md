...
- **Advanced DNS Hunting** provides off-by-default per-detector controls for DGA/NXDOMAIN bursts, beaconing, tunneling, rebinding, and DNS bypass scoring. Exact threat-intelligence matches stay active independently.
...
- DNS Detection Quality follow-through: durable device/domain baselines, sensor-level DNS collection-health detail, and CDN-aware answer-churn analysis. The opt-in DGA/NXDOMAIN burst correlation is shipped with the Advanced DNS Hunting profile.
