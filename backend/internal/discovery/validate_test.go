...
func TestValidateScanTargetRejectsOversizedNumericTarget(t *testing.T) {
	target := strings.Repeat("1,", maxNmapNumericTargetBytes/2) + "1.1.1.1"
	if len(target) <= maxNmapNumericTargetBytes {
		t.Fatalf("test target length = %d, want > %d", len(target), maxNmapNumericTargetBytes)
	}
	if err := ValidateScanTarget(target); err == nil || !strings.Contains(err.Error(), "too long") {
		t.Fatalf("ValidateScanTarget oversized error = %v, want length rejection", err)
	}
}
